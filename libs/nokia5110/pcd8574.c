...
n5110_init(void)
{
	...
	NSS_LOW;
	CMD_MODE;
	SPI2_Send(FUNC_SET | FUNC_SET_EXT);
	SPI2_Send(OPERATION_VOLTAGE | (0x45 & 0x7F));
	SPI2_Send(TEMP_COEFF | 0x02);
	SPI2_Send(BIAS_SYSTEM | 0x03);

	SPI2_Send(FUNC_SET | FUNC_SET_BASIC);
	SPI2_Send(DIS_CONF_NORMAL);
